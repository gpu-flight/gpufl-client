...
CallbackHandler(void *userData, CUpti_CallbackDomain domain, CUpti_CallbackId callbackId, const CUpti_CallbackData *callbackData) {
    switch(domain)
    {
        ...
        {
            // For resource domain, cbdata is CUpti_ResourceData — NOT CUpti_CallbackData
            const CUpti_ResourceData *resourceData = (const CUpti_ResourceData*)callbackData;
                ...
                callbackId == CUPTI_CBID_RESOURCE_MODULE_UNLOAD_STARTING)
            {
                ...
                // resourceDescriptor holds CUpti_ModuleResourceData for module callbacks
                CUpti_ModuleResourceData *modData =
                    (CUpti_ModuleResourceData*)resourceData->resourceDescriptor;

                if (modData) {
                    printf("modData is available — moduleId=%u  cubinSize=%zu\n",
                           modData->moduleId,
                           modData->cubinSize);
                    ...
                    if (modData->pCubin && modData->cubinSize > 0) {
                        char fname[64];
                        snprintf(fname, sizeof(fname), "module_%u.cubin", modData->moduleId);
