...
static bool IsInternalFunction(const std::string& name) {
    if (name.rfind("__device_stub", 0) == 0)
        return true;  // Starts with __device_stub
    ...
    if (name.find("driver") != std::string::npos)
        return true;  // generic driver catch
    return false;
}
...
static std::string SanitizeStackTrace(const std::string& rawTrace) {
    ...
    while (std::getline(ss, segment, '|')) {
        ...
        if (segment == "mainCRTStartup" || segment == "__scrt_common_main" ||
            segment == "__scrt_common_main_seh" || segment == "invoke_main" ||
