...
bool CuptiSass::setMetricsConfig(uint32_t deviceIndex,
    ...
    for (const auto& metricName : metricNames) {
        ...
        if (res != CUPTI_SUCCESS) {
            const char* err;
            cuptiGetResultString(res, &err);
            GFL_LOG_ERROR("[SASS Metrics] Failed to get properties for metric ",
                          metricName, ": ", err);
            continue;
        }
    ...
    CUpti_SassMetricsSetConfig_Params setParams = {
    ...
    GFL_LOG_DEBUG("[SASS Metrics] Configured ", configs.size(),
                  " metrics for device ", deviceIndex);
    return true;
}

bool CuptiSass::unsetMetricsConfig(uint32_t deviceIndex) {
    CUpti_SassMetricsUnsetConfig_Params params = {
    ...
    if (res != CUPTI_SUCCESS) {
        const char* err;
        cuptiGetResultString(res, &err);
        GFL_LOG_ERROR("[SASS Metrics] cuptiSassMetricsUnsetConfig FAILED: ",
                      err);
        return false;
    }
    return true;
...
bool CuptiSass::enableMetrics(void* ctx) {
    CUpti_SassMetricsEnable_Params params = {
...
bool CuptiSass::disableMetrics(void* ctx) {
    CUpti_SassMetricsDisable_Params params = {
...
std::vector<SassMetricData> CuptiSass::flushMetricsData(void* ctx) {
    CUpti_SassMetricsGetDataProperties_Params propParams = {
        CUpti_SassMetricsGetDataProperties_Params_STRUCT_SIZE};
    propParams.ctx = static_cast<CUcontext>(ctx);
    ...
    if (res != CUPTI_SUCCESS) {
        ...
        cuptiGetResultString(res, &err);
        GFL_LOG_ERROR(
            "[SASS Metrics] cuptiSassMetricsGetDataProperties FAILED: ", err);
        return {};
    }
    ...
    std::vector<CUpti_SassMetrics_Data> rawData(
        propParams.numOfPatchedInstructionRecords);
    std::vector<CUpti_SassMetrics_InstanceValue> instanceValues(
        propParams.numOfPatchedInstructionRecords * propParams.numOfInstances);

    for (size_t i = 0; i < propParams.numOfPatchedInstructionRecords; ++i) {
        rawData[i].structSize = sizeof(CUpti_SassMetrics_Data);
        rawData[i].pInstanceValues =
            &instanceValues[i * propParams.numOfInstances];
    }

    CUpti_SassMetricsFlushData_Params flushParams = {
        CUpti_SassMetricsFlushData_Params_STRUCT_SIZE};
    flushParams.ctx = static_cast<CUcontext>(ctx);
    flushParams.numOfPatchedInstructionRecords =
        propParams.numOfPatchedInstructionRecords;
    ...
    for (const auto& raw : rawData) {
        ...
        for (size_t j = 0; j < propParams.numOfInstances; ++j) {
            data.values.push_back({raw.pInstanceValues[j].metricId,
                                   raw.pInstanceValues[j].value});
        }
